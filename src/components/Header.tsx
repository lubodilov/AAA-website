...
export default function Header({ onOpenContact, onOpenSchedule, isScrolled = false }: HeaderProps) {
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 z-50 bg-black/30 backdrop-blur-md border-b border-white/10 transition-all duration-300"
    >
        ...
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center space-x-3 group cursor-pointer">
            <div className="relative">
              <div className="w-10 h-10 bg-gradient-to-br from-red-600 to-red-700 rounded-lg flex items-center justify-center shadow-lg shadow-red-600/25 group-hover:shadow-red-600/40 transition-all duration-300 group-hover:scale-105">
                <span className="text-white font-bold text-lg drop-shadow-lg">UV</span>
              </div>
            ...
            <div className="flex flex-col">
              <span className="text-white font-light text-xl tracking-tight drop-shadow-md">Upgrade Vision</span>
              <span className="text-red-400 text-xs font-extralight tracking-wider uppercase drop-shadow-md">AI Solutions</span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`font-light transition-colors duration-300 drop-shadow-md ${
          ...
          <div className="hidden md:flex items-center space-x-4">
            <button
              ...
              className="group relative bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-2.5 rounded-full hover:from-red-700 hover:to-red-800 transition-all duration-300 flex items-center space-x-2 shadow-lg shadow-red-600/30 hover:shadow-red-600/50 hover:scale-105"
            >
